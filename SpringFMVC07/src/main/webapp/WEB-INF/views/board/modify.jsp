			...
			<tr>
				<td colspan="2" style="text-align: center;">
					<c:if test="${!empty mvo && mvo.memID eq vo.memID}">
						...
						<button type="button" class="btn btn-warning" onclick="location.href='${cpath}/board/remove?idx=${vo.idx}'">삭제</button>
					</c:if>
					<c:if test="${empty mvo || mvo.memID ne vo.memID}">
						<button disabled="disabled" type="submit" class="btn btn-primary">수정</button>
						<button disabled="disabled" type="button" class="btn btn-warning" onclick="location.href='${cpath}/board/remove?idx=${vo.idx}'">삭제</button>
					</c:if>
					<button type="button" class="btn btn-info" onclick="location.href='${cpath}/board/list'">목록</button>
				</td>
			</tr>
		</table>
		</form>
